...
def snp_veritabanini_ana_csvden_olustur(ana_csv_dosya_yolu):
    ...
    try:
        ...
        print(f"HATA: Ana CSV başlığı okunurken bir hata oluştu: {e}")
        return None, None
...
def run_pca_on_input_data(genotype_input, n_components, imputation_strategy='mean'):
    """
    ...
    if X.shape[1] == 0:
        # Bu kritik bir hata olduğu için gösterilmelidir.
        print(f"HATA: PCA için işlenecek Genotip verisi bulunamadı.")
        return None
    ...
    if actual_n_components == 0:
        print(f"HATA: Hesaplanacak geçerli PCA bileşen sayısı 0.")
        return None
...
def load_model(model_path_val):
    ...
    try:
        return joblib.load(model_path_val)
    except FileNotFoundError:
...
def get_target_column_names(y_path):
    ...
    if y_path and os.path.exists(y_path):
        try:
            ...
            if y_df.columns[0].lower().strip() in ['sample', 'sampleid', 'id', 'unnamed: 0']:
...
def make_prediction_and_display(model, features_df, target_names, sample_id):
    """Tahmin yapar ve SADECE nihai sonucu gösterir."""
    try:
        # features_df artık DataFrame olduğu için reshape'e gerek yok
        prediction = model.predict(features_df)
    except Exception as e:
        print(f"HATA: Tahmin sırasında bir hata oluştu: {e}")
        return

    # Sadece nihai sonuçları yazdırmak için bu kısım korunur
    print("\n" + "=" * 55)
    ...
    if sum_of_positives > 0:
        ...
        # Bu bir uyarı ama sonucun kendisiyle ilgili olduğu için önemli
...
if __name__ == "__main__":
    ...
    # UYARI'ları engellemek için çift köşeli parantez kullanılarak DataFrame olarak alınır
    kullanici_pca_ozellikleri_df = final_combined_pca_scores_df.loc[[kullanici_kimligi]]

    # --- ADIM 5: Etnik Köken Tahmini ---
    model = load_model(model_path)
    if model:
        target_names = get_target_column_names(y_column_names_path)
        # Fonksiyona DataFrame gönderilir
        make_prediction_and_display(model, kullanici_pca_ozellikleri_df, target_names, kullanici_kimligi)